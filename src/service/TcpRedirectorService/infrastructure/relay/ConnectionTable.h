 ...
 *
 * ОГРАНИЧЕНИЕ ОБРАТНОГО ПОИСКА (QA-аудит B2).
 * Запись Add() использует составной ключ (src_port + orig_dest_ip), поэтому два
 * одновременно отслеживаемых соединения с одинаковым эфемерным src_port к РАЗНЫМ
 * адресам назначения не затирают друг друга (обе записи сохраняются в цепочке
 * коллизий). Однако ЧТЕНИЕ (Get/GetProxyConfigId/IsTracked/GetInfo) и Remove
 * ищут ТОЛЬКО по src_port: на момент обратного поиска вызывающая сторона
 * (relay при accept'е, WinDivert на обратном пути) знает лишь src_port —
 * оригинальный dst как раз и восстанавливается, поэтому передать его в ключ
 * чтения невозможно. Get() возвращает последнюю добавленную запись для данного
 * src_port (best-effort: свежий SYN обычно соответствует свежему relay-accept);
 * после удаления одной из коллизирующих записей вторая снова становится
 * резолвимой. Полное устранение неоднозначности потребовало бы иной схемы
 * восстановления dst на уровне relay/WinDivert (вне рамок этого модуля).
 * Поведение зафиксировано тестами tests/unit/service/ConnectionTableTest.cpp.
 */
