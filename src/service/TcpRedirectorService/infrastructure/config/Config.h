...
struct WintunSettings {
    ...
    //!< По умолчанию true — не-целевой трафик идёт НАПРЯМУЮ мимо прокси
    //!< (embedded-движок, Option 1): открывает per-flow outbound-сокет к
    //!< оригинальному dst, пиннит его к физическому NIC через IP_UNICAST_IF
    //!< (чтобы SYN не заворачивался обратно в TUN) и качает данные асинхронно
    //!< тем же механизмом, что и PROXY-flow.  При false — прежнее fail-fast
    //!< поведение (DIRECT дропается через tcp_abort).
    //!< IPv4-only; IPv6 по-прежнему блокируется (block_ipv6).
    //!< В external-движке поле не влияет.
    bool direct_passthrough = true;
